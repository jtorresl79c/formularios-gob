...
class TestimonialsDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return datatables()
            ...
            ->editColumn('rating', function (Testimonial $row) {
                ...
                if ($row->image) {
                    if (File::exists(Storage::path($row->image))) {
                        $a = '<img src="' . Storage::url($row->image) . '" style="width:60px; ">';
                    } else {
                        $a = '<img src="' . Storage::url('test_image/350x250.png') . '" style="width:60px;  ">';
                    }
                } else {
                    $a = '<img src="' . Storage::url('test_image/350x250.png') . '" style="width:60px; ">';
                }
                ...
                if ($row->status == 1) {
                    $a = '<div class="form-check form-switch">
                                            <input class="form-check-input chnageStatus" checked type="checkbox" role="switch" id="' . $row->id . '" data-url="' . route('testimonial.status', $row->id) . '">
                                        </div>';
                } else {
                    $a =  '<div class="form-check form-switch">
                                            <input class="form-check-input chnageStatus" type="checkbox" role="switch" id="' . $row->id . '" data-url="' . route('testimonial.status', $row->id) . '">
                                        </div>';
                }
                return $a;
            })
    ...
    $canCreateTestimonial = \Auth::user()->can('create-testimonial');
    $canExportTestimonial = \Auth::user()->can('export-testimonial');
    $buttonsConfig = [];
    ...
    if ($canCreateTestimonial) {
        $buttonsConfig[] = [
            ...
            'action' => " function (e, dt, node, config) { window.location = '" . route('testimonial.create') . "'; }",
        ];
    }
    $exportButtonConfig = [];

    if ($canExportTestimonial) {
        $exportButtonConfig = [
            'extend' => 'collection',
