                        ...
                        <div class="card-body form-card-body">
                            <form action="{{ route('forms.fill.store', $form->id) }}" method="POST"
                                    ...
                                    <div class="col cap">
                                        @if (UtilityFacades::getsettings('captcha_enable') == 'on')
                                            @if (UtilityFacades::getsettings('captcha') == 'hcaptcha')
                                                {!! HCaptcha::renderJs() !!}
                                                <small class="text-danger font-weight-bold">{{ __('Note :- reCAPTCHA Is required') }}</small>
                                ...
                                @if (!isset($form_value) && $form->payment_status == 1)
                                    ...
                                    @elseif(!isset($form_value) && $form->payment_type == 'mollie')
                                        <div class="mollie">
                                            <p>{{ __('Make Payment') }}</p>
                                            {!! Form::hidden('payment_id', null, ['id' => 'payment_id']) !!}
                                            <h5>{{ __('Payble Amount') }} : {{ $form->currency_symbol }}
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('form').submit(function(event) {
                ...
                $.ajax({
                    url: $(this).attr('action'), // Form action URLcaptcyh
                    ...
                    error: function(xhr, status, error) {
                        if (xhr.responseJSON && xhr.responseJSON.errors) {
                            ...
                            if (errors['g-recaptcha-response']) {
                                // Display the error message next to the captcha field
                                console.log($('.captcha-error '));
                                $('.captcha-error ').after('<div class="text-danger">' + errors[
                                    'g-recaptcha-response'][0] + '</div>');
                            }
                        }
                    }
                });
