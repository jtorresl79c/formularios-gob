<div class="mx-0 mt-5 row">
    <div class="mx-auto col-md-5">
        ...
        <div class="card">
            ...
            <div class="card-body">
                {!! Form::modal($form, [
                    'route' => ['forms.fill.store', $form->id],
                    'method' => 'PUT',
                    'id' => 'fill-form',
                ...
                @if (session()->has('success'))
                    <div class="text-center gallery" id="success_loader">
                        {!! Form::image(asset('assets/images/success.gif'), null, []) !!}
                        <br> <br>
                ...
                @else
                    <div class="row">
                        @foreach ($array as $row_key => $row)
                                ...
                                <div class="form-group {{ $col }} ">
                                    ...
                                    @foreach ($row->values as $key => $options)
                                        ...
                                        <div class="{{ $class }}">
                                            ...
                                            {!! Form::label($row->name . '_' . $key, $options->label, ['class' => $l_class]) !!}
                                        </div>
                                    @endforeach
                                </div>
                                            ...
                                            @if (is_array($row->value))
                                                <div class="row">
                                                    @foreach ($row->value as $img)
                                                        <div class="col-3">
                                                            {!! Form::image(Storage::exists($img) ? Storage::url($img) : Storage::url('appLogo/78x78.png'), null, [
                            ...
                            @elseif($row->type == 'select' || $row->type == 'autocomplete')
                                <div class="form-group {{ $col }}">
                                    @php
                                        $attr = ['class' => 'form-control', 'data-trigger', 'id' => $row->name];
                                        if ($row->required) {
                                            $attr['required'] = 'required';
                                        }
                                    ...
                                    @if (isset($row->is_parent) && $row->is_parent == 'true')
                                        ...
                                        <div class="input-group">
                                            ...
                                            <div class="input-group-append">
                                                {{ Form::button(__('Add Photo'), ['class' => 'btn btn-outline-primary open-photo']) }}
                                            </div>
                                        </div>
                                    @else
                                        {{ Form::label($row->name, $row->label) }}@if ($row->required)
                            ...
                            @elseif($row->type == 'starRating')
                                <div class="form-group {{ $col }}">
                                    ...
                                    {!! Form::hidden($row->name, $value, ['class' => 'calculate', 'data-star' => $num_of_star]) !!}
                                </div>
                            @endif
                        @endforeach
                    ...
                    @if (isset($form_value))
                        ...
                        <div class="pb-0 mt-3 form-actions">
                            ...
                            {{ Form::button(__('Submit'), ['type' => 'submit','id' => 'save-form', 'class' => 'btn btn-primary mr-1 float-end']) }}
                        </div>
                    @else
                        <div class="cap">
                            @if (setting('captcha') == 'hcaptcha')
